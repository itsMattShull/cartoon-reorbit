...
const BULLET_LIFE_TICKS = 66
const RESPAWN_TICKS     = 96   // invulnerable spawn-in after losing a life
const SAFE_SPAWN_R2     = 90 * 90 // don't respawn if an asteroid is within this squared distance
const POWERUP_RADIUS    = 13
const POWERUP_LIFE      = 600  // 10s before an uncollected power-up despawns
...
export function shipRadius(st) {
  ...
  return st.puTicks[PU_GREEN] > 0 ? SHIP_RADIUS / 2 : SHIP_RADIUS
}
...
export function step(st, input) {
  ...
  if (!s.alive) {
    ...
    if (s.cooldown === 0 && st.bullets.length < MAX_BULLETS) {
      const ca = headingCos(s.heading), sa = headingSin(s.heading)
      st.bullets.push({
        x: s.x + ca * 14, y: s.y + sa * 14,
        vx: s.vx + ca * BULLET_SPEED,
        vy: s.vy + sa * BULLET_SPEED,
        life: BULLET_LIFE_TICKS
      })
      s.cooldown = fireInterval(st)
      st.events.push({ t: 'shoot' })
    }
  }
  ...
  for (let i = st.powerups.length - 1; i >= 0; i--) {
    ...
    if (s.alive && hits(s.x, s.y, shipRadius(st), p.x, p.y, POWERUP_RADIUS)) {

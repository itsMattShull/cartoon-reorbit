...
export const SHIPS = [
  {
    ...
    // spriteTurns is how far the source art is rotated from "nose pointing right", in turns.
    // All three ships are drawn facing left, hence 0.5 (half a turn) across the board. Swapping
    // in art drawn at a different angle is a one-number change here, never a renderer change.
    spriteTurns: 0.5,
    weapon: { fireIntervalTicks: 26, bulletSpeed: 7.6, bulletLifeTicks: 300, pellets: 1, spreadSteps: 0, muzzleRadii: 2.25 }
  },
  {
    id: SHIP_SCAMPER,
    name: 'S.C.A.M.P.E.R.',
    ...
    spriteTurns: 0.5,
    weapon: { fireIntervalTicks: 21, bulletSpeed: 5.6, bulletLifeTicks: 15, pellets: 5, spreadSteps: 96, muzzleRadii: 2.25 }
  },
  {
    id: SHIP_CASUAL,
    name: 'C.A.S.U.A.L.',
    ...
    spriteTurns: 0.5,
    weapon: { fireIntervalTicks: 13, bulletSpeed: 6.2, bulletLifeTicks: 66, pellets: 1, spreadSteps: 0, muzzleRadii: 2.25 }
  }
]
...
export const DEFAULT_CONFIG = {
  ...
  // Where a shot leaves the ship, as a multiple of shipRadius. The sprite is drawn 4.8x the
  // collision radius across, so its nose sits 2.4 radii out; 2.25 puts the muzzle just inside
  // that, so projectiles emerge FROM the front of the art rather than the middle of it.
  // Being a multiple rather than an absolute distance means it tracks an admin-retuned radius.
  muzzleRadii: 2.25,
...
export function normalizeConfig(raw) {
  ...
  return {
    ...
    muzzleRadii:          num(c.muzzleRadii, 0, 4, 2.25),
...
function fireVolley(st) {
  ...
  const muzzle = c.shipRadius * c.muzzleRadii

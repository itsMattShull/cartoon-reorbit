...
/* ── Clocks ──────────────────────────────────────────────────────────────────────────────
 * The clock arithmetic lives here, as pure functions, rather than inside the socket runtime.
 * Two reasons, and the second is the one that matters: it is the only part of this game where
 * a bug is worth real points, and the runtime imports Prisma and Redis — so a test that
 * reaches into it never drains the event loop and hangs. Pure functions over plain numbers
 * are testable without booting a database.
 *
 * `turnIdx` is 0 for White, 1 for Black. `turnStartedAt` is an absolute epoch ms set by the
 * server when a position is broadcast. Latency is charged to the mover, which is why an
 * increment is mandatory below a 3-minute base time — the increment IS the lag compensation,
 * and a separate per-move allowance would just be a per-move gift of free time that a client
 * can farm by delaying every move to just inside it.
 */

/** Remaining ms for both sides right now. Pure: never mutates `clocks`. */
export function liveClocks ({ clocks, turnIdx, turnStartedAt }, now = Date.now()) {
  const out = [clocks[0], clocks[1]]
  if (turnStartedAt) {
    out[turnIdx] = Math.max(0, out[turnIdx] - Math.max(0, now - turnStartedAt))
  }
  return out
}

/**
 * Charges the mover for the time they took and credits their increment.
 *
 * Returns { ms, flagged, spent }. When `flagged` is true the clock is 0 and NO increment has
 * been credited — a player who has already run out must not be handed three fresh seconds, or
 * the flag never fires at all.
 *
 * Must only ever be called for a move that actually happened. Charging before validating means
 * an illegal move also collects the increment, and since a rejected move leaves turnStartedAt
 * where it was, a client spamming nonsense would gain time without end.
 */
export function chargeClock ({ clockMs, turnStartedAt, incrementSeconds }, now = Date.now()) {
  const spent = Math.max(0, now - turnStartedAt)
  const left = clockMs - spent
  if (left <= 0) return { ms: 0, flagged: true, spent }
  return { ms: left + incrementSeconds * 1000, flagged: false, spent }
}

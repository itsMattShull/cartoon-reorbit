...
// pursue. The base rule: a user may not bid if they currently own >= N unburned
// copies, or have *received* >= N distinct copies in the last 30 days. N is
// `featuredOwnLimit`, chosen per listing at creation time (Standard = 2, the
// long-standing default; Strict = 1) rather than a single hardcoded value.
//
...
// combined across BOTH editions — owning 1 First + 1 Second edition counts as 2
// toward the same limit, and likewise for the 30-day received count. This
// combined counting applies ONLY when the auctioned cToon is a Second Edition;
// a featured auction of a plain / First Edition cToon keeps the single-ctoon
// rule.
import { createError } from 'h3'

export const FEATURED_THIRTY_DAYS_MS = 30 * 24 * 60 * 60 * 1000

/// Values selectable for `AuctionOnly.featuredOwnLimit` / `Auction.featuredOwnLimit`.
export const FEATURED_OWN_LIMIT_OPTIONS = [1, 2]
export const DEFAULT_FEATURED_OWN_LIMIT = 2
 ...
 * @param {string} userId
 * @param {{ ctoonId?: string|null, isSecondEdition?: boolean|null, relatedFirstEditionId?: string|null }} ctoon
 * @param {number} [ownLimit] threshold N from the listing; defaults to the standard rule
 * @returns {Promise<{ eligible: boolean, reason?: 'own'|'recent' }>}
 */
export async function checkFeaturedEligibility(client, userId, ctoon, ownLimit = DEFAULT_FEATURED_OWN_LIMIT) {
  ...
  if (currentOwned >= ownLimit) return { eligible: false, reason: 'own' }
  ...
  if (recent.length >= ownLimit) return { eligible: false, reason: 'recent' }
 ...
 * @param {string} userId
 * @param {{ isFeatured?: boolean|null, featuredOwnLimit?: number|null, ctoon: object }} auction
 */
export async function assertFeaturedEligibility(client, userId, { isFeatured, featuredOwnLimit, ctoon }) {
  if (!isFeatured) return
  const { eligible } = await checkFeaturedEligibility(client, userId, ctoon, featuredOwnLimit ?? DEFAULT_FEATURED_OWN_LIMIT)
  if (!eligible) {
    throw createError({
      statusCode: 403,

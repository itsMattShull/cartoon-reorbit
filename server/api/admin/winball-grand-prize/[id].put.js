// server/api/admin/winball-grand-prize/[id].put.js
...
export default defineEventHandler(async (event) => {
  ...
  const schedule = await db.winballGrandPrizeSchedule.findUnique({ where: { id }, select: { id: true } })
  ...
  const row = await db.winballGrandPrizeSchedule.update({
    where: { id },
    data: { ctoonId, startsAt: startsAtUtc },

// server/api/admin/barcode-species.post.js
...
export default defineEventHandler(async (event) => {
  ...
  // Parse multipart with fields + 3 images
  ...
  // Auto-assign speciesIndex to next available for this config
  const last = await db.speciesBaseStats.findFirst({ where: { configId: cfg.id }, orderBy: { speciesIndex: 'desc' }, select: { speciesIndex: true } })
  const nextIndex = (last?.speciesIndex ?? 0) + 1

  // Save images to monsters/{speciesName}
  const safeName = name.replace(/[^A-Za-z0-9._ -]/g, '').trim() || 'monster'
  ...
  async function saveOne(label, part) {
    ...
    return process.env.NODE_ENV === 'production'
      ? `/images/monsters/${safeName}/${filename}`
      : `/monsters/${safeName}/${filename}`
  }
  ...
  const created = await db.speciesBaseStats.create({
    data: {
      configId: cfg.id,
      speciesIndex: nextIndex,
      name,
      type,
      rarity,
      baseHp,
  ...
  await logAdminChange(db, { userId: me.id, area: 'SpeciesBaseStats', key: 'create', prevValue: null, newValue: { configId: cfg.id, speciesIndex: created.speciesIndex } })

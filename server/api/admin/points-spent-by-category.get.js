// server/api/admin/points-spent-by-category.get.js
...
export default defineEventHandler(async (event) => {
  ...
  // 3) Cache check
  const cacheKey = `admin:points-spent-by-category:${timeframe}:${groupBy}`
  try {
    const hit = await redis.get(cacheKey)
    if (hit) return JSON.parse(hit)
  } catch {}

  const result = await computeCategoryBreakdown({ direction: 'decrease', timeframe, groupBy })

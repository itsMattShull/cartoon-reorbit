...
const SLOT_COLUMNS = {
  blossom: 'flappyBlossomImagePath',
  bubbles: 'flappyBubblesImagePath',
  buttercup: 'flappyButtercupImagePath',
  city: 'flappyCityImagePath'
}

// Sprites sit at ~110 world units tall on a canvas that never exceeds ~2x that in device
// pixels; the city is a full-width backdrop. Anything larger is wasted bytes on every load.
const SLOT_MAX_WIDTH = { blossom: 320, bubbles: 320, buttercup: 320, city: 1024 }
...
export default defineEventHandler(async (event) => {
  ...
  // this origin, so the extension is derived from the actual bytes and the whole name is
  // built server-side. SVG is intentionally unsupported: sniffImageType doesn't recognize it,
  // and it would execute script on direct navigation.
  ...
  const filename = `flappy-${slot}-${Date.now()}.webp`
  ...
  const before = await db.gameConfig.findUnique({ where: { gameName: 'FlappyPowerpuff' } })
  await db.gameConfig.upsert({
    where: { gameName: 'FlappyPowerpuff' },
    create: { gameName: 'FlappyPowerpuff', [column]: assetPath },
  ...
  if ((before?.[column] ?? null) !== assetPath) {
    await logAdminChange(db, {
      userId: me.id,
      area: 'GameConfig:FlappyPowerpuff',
      key: column,

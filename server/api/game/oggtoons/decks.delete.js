// File: server/api/game/oggtoons/decks.delete.js
...
export default defineEventHandler(async (event) => {
  ...
  if (!id || typeof id !== 'string') {
    throw createError({ statusCode: 400, statusMessage: 'Must provide a deck `id` to delete' })
  }

  // 3) Ensure the deck exists and belongs to this user
  const deck = await prisma.ogGtoonDeck.findUnique({ where: { id } })
  ...
  await prisma.ogGtoonDeck.delete({ where: { id } })

-- DropForeignKey (tolerate missing tables/constraints for shadow DB)
DO $$ BEGIN
...
-- AlterTable
ALTER TABLE "ClashGame" ADD COLUMN     "isTournament" BOOLEAN NOT NULL DEFAULT false,
ADD COLUMN     "player1DeckSnapshot" JSONB,
ADD COLUMN     "player2DeckSnapshot" JSONB;

-- AddForeignKey (tolerate missing tables/constraints for shadow DB)
DO $$ BEGIN
  IF to_regclass('"CZoneSearchPrize"') IS NOT NULL THEN
    IF NOT EXISTS (
      SELECT 1 FROM pg_constraint
      ...
      ALTER TABLE "CZoneSearchPrize"
        ...
        FOREIGN KEY ("ctoonId") REFERENCES "Ctoon"("id") ON DELETE CASCADE ON UPDATE CASCADE;
    END IF;
  END IF;
END $$;

    ...
    <template #main-content>
      <div class="tournament-detail overflow-y-auto h-full px-4 py-3">
        <div class="mb-3">
          <NuxtLink
            to="/newsite/gtoons"
            class="text-sm text-indigo-300 hover:text-indigo-200 underline"
          >
            ← Back to gToons
          </NuxtLink>
        </div>

        <div v-if="loading" class="text-gray-400">Loading tournament...</div>

        <template v-else>
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <!-- Tournament info -->
            <div class="order-1 lg:col-start-1 bg-white/10 border border-white/20 rounded-lg p-4">
              ...
              <div class="mt-4 flex flex-wrap gap-2">
                ...
                <div v-else class="text-sm text-gray-400">Opt-in is closed.</div>
              </div>
            </div>

            <!-- Your current matches -->
            <div class="order-2 lg:col-start-2 lg:row-span-2 bg-white/10 border border-white/20 rounded-lg p-4">
              <h2 class="text-base font-semibold mb-2">Your current match</h2>
              <div v-if="!myMatches.length" class="text-sm text-gray-400">No active matches right now.</div>
              <div v-else class="space-y-3">
                <div v-for="match in myMatches" :key="match.id" class="border border-white/20 rounded p-3">
                  <div class="text-sm font-semibold">
                    {{ match.playerAName }} vs {{ match.playerBName }}
                  </div>
                  <div class="text-xs text-gray-400 mt-1">{{ match.stage }} Round {{ match.roundNumber }}</div>
                  <div class="text-sm mt-2">Score: {{ scoreLabel(match) }}</div>
                  <div v-if="match.suddenDeathActive" class="text-xs text-amber-400 mt-1">Sudden death in progress</div>
                  <div v-if="match.tiebreakNotes" class="text-xs text-red-400 mt-1">{{ match.tiebreakNotes }}</div>
                  <div class="mt-3">
                    <button
            ...
            <!-- Standings -->
            <div class="order-3 lg:col-start-1 bg-white/10 border border-white/20 rounded-lg p-4">
              <h2 class="text-base font-semibold mb-3">Standings</h2>
              <div v-if="showBracketStats" class="overflow-x-auto mb-4">
                <div class="text-xs font-semibold text-gray-400 mb-2">Bracket</div>
                <table class="min-w-full text-xs">
                  ...
                  <tbody>
                    <tr v-for="row in standings" :key="`bracket-${row.userId}`" class="border-t border-white/10">
              ...
              <div v-if="showSwissStats" class="overflow-x-auto">
                <div class="text-xs font-semibold text-gray-400 mb-2">Swiss</div>
                <table class="min-w-full text-xs">
                  <thead>
                    <tr class="text-left text-gray-400">
                      <th class="py-1 pr-2">Player</th>
                  ...
                  <tbody>
                    <tr v-for="row in standings" :key="`swiss-${row.userId}`" class="border-t border-white/10">
                      <td class="py-1 pr-2">{{ row.username }}</td>
          ...
          <div class="mt-4 space-y-4">
            <div v-if="swissRounds.length" class="bg-white/10 border border-white/20 rounded-lg p-4">
              <h2 class="text-base font-semibold mb-3">Swiss Rounds</h2>
              <div class="space-y-3">
                <div v-for="round in swissRounds" :key="round.roundNumber" class="border border-white/10 rounded p-3">
                  ...
                  <div class="space-y-2">
                    <div v-for="match in round.matches" :key="match.id" class="text-sm flex items-center justify-between">
                      <div>
                        {{ match.playerAName }} vs {{ match.playerBName }}
                        <span v-if="match.status === 'BYE'" class="text-xs text-gray-400 ml-2">BYE</span>
                      </div>
                      <div class="text-xs text-gray-400">{{ scoreLabel(match) }} · {{ outcomeLabel(match) }}</div>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div v-if="bracketRounds.length" class="bg-white/10 border border-white/20 rounded-lg p-4">
              <h2 class="text-base font-semibold mb-3">Bracket</h2>
              <div class="flex flex-wrap gap-4 overflow-x-auto">
                <div v-for="round in bracketRounds" :key="round.roundNumber" class="min-w-[200px]">
                  <div class="text-sm font-semibold mb-2">Round {{ round.roundNumber }}</div>
                  <div class="space-y-3">
                    <div v-for="match in round.matches" :key="match.id" class="border border-white/20 rounded p-3 text-sm">
                      <div class="font-semibold">{{ match.playerAName }} vs {{ match.playerBName }}</div>
                      <div class="text-xs text-gray-400 mt-1">Score: {{ scoreLabel(match) }}</div>
                      <div v-if="match.needsTiebreak" class="text-xs text-amber-400 mt-1">Sudden death in progress</div>
                      <div v-if="match.tiebreakNotes" class="text-xs text-red-400 mt-1">{{ match.tiebreakNotes }}</div>
                    </div>
                  </div>
                </div>

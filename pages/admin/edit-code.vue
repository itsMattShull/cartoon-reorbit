...
onMounted(async () => {
  try {
    ...
    // 2) fetch codes list (now must include maxClaims & full rewards)
    ...
    maxClaims.value = entry.maxClaims         // now populated
    expiresAt.value = entry.expiresAt
      ? isoToCSTLocal(entry.expiresAt)       // CST timezone
      : ''
    points.value    = entry.rewards
      .reduce((sum, r) => sum + (r.points || 0), 0)

    // 4) map existing cToon rewards to names
    ctoonRewards.value = entry.rewards.flatMap(r =>
      (r.ctoons || []).map(rc => {
        const found = ctoonOptions.value.find(ct => ct.id === rc.ctoonId)
        return {
          ctoonName: found ? found.name : '',
          quantity:  rc.quantity
        }
      })
    )

    // if no existing rewards, initialize one blank row
...
function removeCtoonReward(i) {
  ctoonRewards.value.splice(i, 1)
}
...
async function submitForm() {
  ...
  // build payload mapping names back to IDs
  const validCtoons = []

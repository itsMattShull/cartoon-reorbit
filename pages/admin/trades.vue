...
// Fetch
const { data: tradesData } = await useAsyncData('trades', () => $fetch('/api/admin/trades'))
...
const users = computed(() => {
  const set = new Set()
  trades.value.forEach(t => { set.add(t.initiator.username); set.add(t.recipient.username) })

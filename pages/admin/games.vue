    ...
    <div class="bg-white rounded-lg shadow-md p-6 max-w-2xl mx-auto space-y-12">
      ...
      <section>
        ...
        <div class="mb-6">
          ...
          <input
            type="number"
            v-model.number="globalDailyPointLimit"
            class="input"
          />
        </div>
        <button
          ...
          >
            ...
            >
              <img
                :src="c.assetPath"
                alt=""
                class="w-8 h-8 rounded mr-3 object-cover border"
              />
              <div>
      ...
      <section>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
          <div>
            ...
            <input
              type="number"
              v-model.number="clashPointsPerWin"
              class="input"
            />
          </div>
        </div>
...
function onSearchInput() {
  ...
  if (!searchTerm.value.trim()) clearSelection()
}
...
onMounted(async () => {
  try {
    ...
    // load all cToons for autocomplete
    ...
    window.addEventListener('click', e => {
      ...
      if (el && !el.contains(e.target)) showDropdown.value = false
    })
    ...
    console.error(err)
    toastMessage.value = 'Failed to load configurations'
    toastType.value    = 'error'
  }
})
...
async function saveGlobalConfig() {
  ...
  try {
    ...
    toastMessage.value = 'Global settings saved!'
    toastType.value    = 'success'
  } catch {
    toastMessage.value = 'Error saving global settings'
    toastType.value    = 'error'
  } finally {
...
async function saveWinballConfig() {
  ...
  try {
    ...
    toastMessage.value = 'Winball settings saved!'
    toastType.value    = 'success'
  } catch {
    toastMessage.value = 'Error saving Winball settings'
    toastType.value    = 'error'
  } finally {
...
async function saveClashConfig() {
  ...
  try {
    ...
    toastMessage.value = 'Clash settings saved!'
    toastType.value    = 'success'
  } catch {
    toastMessage.value = 'Error saving Clash settings'
    toastType.value    = 'error'
  } finally {

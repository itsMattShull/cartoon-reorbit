<template>
  <div class="tab-content overflow-y-auto h-full px-4 py-3">
    <div class="flex flex-col sm:flex-row sm:items-center gap-2 mb-4">
      <h2 class="text-xl font-bold">gToons Clash Tournaments</h2>
      <div class="text-sm text-gray-400 sm:ml-auto">Opt in and battle for the crown.</div>
    </div>

    <div v-if="loading" class="text-gray-400">Loading tournaments...</div>
    <div v-else-if="!tournaments.length" class="text-gray-400">No tournaments yet.</div>

    <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-3">
      <div
        v-for="t in tournaments"
        :key="t.id"
        class="bg-white/10 border border-white/20 rounded-lg p-4 flex flex-col gap-2"
      >
        <div class="flex items-start justify-between gap-2">
          <div class="text-base font-semibold">{{ t.name }}</div>
          <span class="text-xs px-2 py-1 rounded-full flex-shrink-0" :class="statusClass(t.status)">
        ...
        <div class="text-sm text-gray-300">
          ...
          <div v-if="countdownText(t)" class="mt-1 text-xs text-indigo-300">
            {{ countdownText(t) }}
          </div>
        </div>

        <NuxtLink
          :to="`/newsite/tournaments/${t.id}`"
...
onMounted(() => {
  load()
  timer = setInterval(() => { nowTs.value = Date.now() }, 60000)
})

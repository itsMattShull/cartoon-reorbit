  ...
  try {
    ...
    for (const toon of (zone?.toons || [])) {
      try {
        ...
        const baseW = toon.width  || img.naturalWidth
        const baseH = toon.height || img.naturalHeight
        const scale = toon.sizeScale || 1
        ctx.drawImage(img, toon.x, toon.y, baseW * scale, baseH * scale)

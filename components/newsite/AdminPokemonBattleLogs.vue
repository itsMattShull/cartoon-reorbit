<template>
  <div class="admin-pokemonbattle-logs bg-gray-50">
    <div class="p-2 space-y-2 text-xs">
      <h1 class="text-base font-bold mb-1">Pokemon: Fire, Water, Grass! Logs</h1>
      ...
      <p class="text-[11px] text-gray-600 mb-1">
        ...
        giveaway for a script — humans on a 12-second timer do not choose in 40ms.
      </p>
...
function isFlagged(m) {
  return m.sameIp || m.sameVisitorId || ['pair_limit', 'same_device', 'banned', 'self_match'].includes(m.suppressReason)
}
...
async function load() {
  loading.value = true
  try {
    const res = await $fetch('/api/admin/pokemonbattle-matches', {
      query: {
        page: page.value,

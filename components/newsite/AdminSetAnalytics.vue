<template>
  <div class="admin-set-analytics bg-gray-50 text-xs text-gray-900">
    <div class="p-2">
      <div class="bg-white rounded-lg shadow p-3">
        <h1 class="text-base font-semibold mb-2 text-gray-900">Set Analytics</h1>
              ...
              <table v-else class="w-full text-[11px]">
                <thead>
                  <tr class="text-left border-b bg-gray-100">
                    ...
                    <th class="px-1.5 py-1 text-right text-gray-900">cMart</th>
                    <th class="px-1.5 py-1 text-right text-gray-900">Pack</th>
                ...
                <tbody>
                  ...
                  >
                    ...
                    <td class="px-1.5 py-1 text-right tabular-nums text-gray-900">{{ r.cmart.toLocaleString() }}</td>
                    <td class="px-1.5 py-1 text-right tabular-nums text-gray-900">{{ r.pack.toLocaleString() }}</td>
                ...
                <tfoot>
                  <tr class="border-t-2 font-semibold">
                    ...
                    <td class="px-1.5 pt-1.5 text-right tabular-nums text-gray-900">{{ totalCmart.toLocaleString() }}</td>
                    <td class="px-1.5 pt-1.5 text-right tabular-nums text-gray-900">{{ totalPack.toLocaleString() }}</td>
...
const totalCmart = computed(() => rarityBreakdown.value.reduce((s, r) => s + r.cmart, 0))
const totalPack = computed(() => rarityBreakdown.value.reduce((s, r) => s + r.pack, 0))
...
async function fetchData(refresh = false) {
  ...
  try {
    ...
    const data = await $fetch(`/api/admin/set-analytics?${params.toString()}`)
    ...
    if (barChart && rarityBreakdown.value.length) {
      ...
      barChart.data.datasets = [
        { label: 'cMart', data: rarityBreakdown.value.map(r => r.cmart), backgroundColor: '#3B82F6' },
        { label: 'Pack',  data: rarityBreakdown.value.map(r => r.pack),  backgroundColor: '#10B981' },
